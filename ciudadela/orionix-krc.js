...
(function () {
  ...
  const WHATSAPP_NUM = '34000000000'; // Kevin actualiza su número aquí
  const PAYPALEMAIL = 'kevin.ruiz.bolsota@gmail.com';
  const KRC_RATE = 10; // 1€ = 10 KRC al ganar | 100 KRC = 1€ al gastar
  ...
  function krcToEuros(krc) { return (krc / 100).toFixed(2); }
  ...
  // ── Carrito (usa el existente o crea uno) -----------------------
  function getCart() {
...
.krc-badge-float {
  ...
  letter-spacing: .1em; color: #e8c66a;
...
.krc-badge-float .krc-icon { font-size: 1rem; }
...
.krc-modal-title {
  ...
  color: #e8c66a; letter-spacing: .15em; text-align: center;
  margin-bottom: 6px; text-shadow: 0 0 20px rgba(232,200,106,0.4);
}
.krc-modal-sub {
  text-align: center; color: rgba(240,232,208,0.5);
  font-size: .9rem; font-style: italic; margin-bottom: 24px;
}
.krc-divider {
  height: 1px; background: linear-gradient(90deg,transparent,rgba(232,200,106,0.3),transparent);
  margin: 20px 0;
}
.krc-order-summary {
  background: rgba(255,255,255,0.03); border: 1px solid rgba(232,200,106,0.12);
...
.krc-order-total {
  ...
  color: #e8c66a; margin-top: 10px; padding-top: 10px;
...
.krc-field input, .krc-field textarea {
  width: 100%; padding: 12px 14px; background: rgba(255,255,255,0.04);
  border: 1px solid rgba(232,200,106,0.2); border-radius: 10px;
  color: #f0e8d0; font-family: inherit; font-size: 1rem; outline: none;
  transition: border-color .2s;
}
.krc-field input:focus, .krc-field textarea:focus { border-color: #e8c86a; }
.krc-methods { display: flex; flex-direction: column; gap: 12px; margin: 20px 0; }
.krc-method {
  border: 2px solid rgba(232,200,106,0.15); border-radius: 14px;
...
.krc-method-name { font-family: 'Cinzel', serif; font-size: .85rem; font-weight: 700; color: #e8c66a; letter-spacing: .1em; }
...
.krc-method-badge {
  ...
  background: rgba(232,200,106,0.15); color: #e8c86a; white-space: nowrap;
}
.krc-method.krc-coin-method {
  background: linear-gradient(135deg, rgba(232,200,106,0.08), rgba(184,154,64,0.04));
  border-color: rgba(232,200,106,0.3);
}
.krc-coin-balance {
  text-align: center; padding: 12px;
  background: rgba(232,200,106,0.06); border-radius: 10px;
  font-family: 'Cinzel', serif; margin-top: 8px;
}
.krc-coin-balance .krc-big-num { font-size: 1.8rem; color: #e8c86a; font-weight: 900; }
...
.krc-btn-confirm {
  ...
  background: linear-gradient(135deg, #e8c86a, #b89a40);
  color: #050a14; transition: all .3s; margin-top: 8px;
}
...
.krc-btn-cancel {
  width: 100%; padding: 12px; border: 1px solid rgba(232,200,106,0.2);
  border-radius: 12px; background: transparent; color: rgba(240,232,208,0.5);
  font-family: 'Cinzel', serif; font-size: .7rem; letter-spacing: .1em;
  cursor: pointer; transition: all .25s; margin-top: 8px;
}
.krc-btn-cancel:hover { border-color: rgba(232,200,106,0.4); color: #e8c86a; }
#krc-toast {
  position: fixed; bottom: 28px; left: 50%;
  transform: translateX(-50%) translateY(100px);
  background: rgba(5,10,24,.97); border: 1px solid rgba(232,200,106,0.4);
  color: #e8c66a; font-family: 'Cinzel', serif; font-size: .65rem;
  letter-spacing: .1em; padding: 14px 28px; border-radius: 12px;
  z-index: 9999; transition: transform .35s;
  box-shadow: 0 0 30px rgba(232,200,106,0.2); text-align: center;
}
...
.krc-success-title { font-family: 'Cinzel', serif; font-size: 1.3rem; color: #e8c86a; margin-bottom: 10px; }
.krc-success-msg { color: rgba(240,232,208,0.6); font-size: 1rem; line-height: 1.6; margin-bottom: 20px; }
.krc-earned { display: inline-block; padding: 10px 24px; background: rgba(232,200,106,0.12); border: 1px solid rgba(232,200,106,0.3); border-radius: 30px; font-family: 'Cinzel', serif; font-size: .85rem; color: #e8c86a; margin-bottom: 20px; }
    `;
  ...
  function buildBadge() {
    ...
    badge.innerHTML = '<span class="krc-icon">💢</span><span data-krc-balance>' + getKRC().toLocaleString('es-ES') + ' KRC</span>';
    badge.title = 'Tu saldo de KRC — haz clic para ver cómo usarlos';
    badge.addEventListener('click', function () {
      showKRCToast('💢 Tienes ' + getKRC().toLocaleString('es-ES') + ' KRC �- úsalos al pagar en la tienda');
    });
  ...
  function openCheckout(productData) {
    ...
    var itemsHtml = cart.map(function (i) {
      return '<div class="krc-order-item"><span>' + (i.nombre || i.title) + ' x' + (i.qty || 1) + '</span><span>€</span>' + parseFloat(i.precio || i.price || 0).toFixed(2) + '</span></div>';
    }).join('');

    box.innerHTML = `
      <div class="krc-modal-title">✦ CHECKOUT ORIONIX |
      <div class="krc-modal-sub">Elige cómo quieres pagar</div>

      <div class="krc-order-summary">
        ${itemsHtml}
        <div class="krc-order-total"><span>TOTAL</span><span>€${total.toFixed(2)}</span></div>
      </div>

      <div class="krc-field">
        <label>Tu nombre</label>
        <input type="text" id="krc-nombre" placeholder="Kevin Ruiz" value="${sesion.nombre || ''}">
      </div>
      <div class="krc-field">
        <label>Tu Telegram (para coordinar entrega)</label>
        <input type="text" id="krc-telegram" placeholder="@tu_telegram">
      </div>
      ...
      <div class="krc-methods">
        ...
        <div class="krc-method krc-coin-method ${canPayKRC ? '' : 'disabled'}" data-method="krc" onclick="window.ORIONIX_KRC.selectMethod(this,'krc')">
          ...
          <div class="krc-method-info">
            <div class="krc-method-name">KRC</div>
            <div class="krc-method-desc">${canPayKRC ? 'Pagar con ' + krcEquiv + ' KRC de tu saldo' : 'Saldo insuficiente (' + krcBalance + '/' + krcEquiv + ' KRC)'}</div>
          </div>
          <div class="krc-method-badge">${krcBalance.toLocaleString('es-ES')} KRC</div>
        </div>

        <div class="krc-method" data-method="whatsapp" onclick="window.ORIONIX_KRC.selectMethod(this,'whatsapp')">
          <span class="krc-method-icon">📺</span>
          <div class="krc-method-info">
            <div class="krc-method-name">WhatsApp</div>
            <div class="krc-method-desc">Coordina el pago directamente con ORIONIX</div>
          </div>
          <div class="krc-method-badge">VIT</div>
        </div>

        <div class="krc-method" data-method="paypal" onclick="window.ORIONIX_KRC.selectMethod(this,'paypal')">
          <span class="krc-method-icon">💷</span>
          <div class="krc-method-info">
            <div class="krc-method-name">PayPal</div>
            <div class="krc-method-desc">Pago seguro con tarjeta o cuenta PayPal</div>
          </div>
          <div class="krc-method-badge">SEGURO�</div>
        </div>

      </div>

      <div class="krc-coin-balance" id="krc-coins-earned-preview" style="display:none">
        <div class="krc-small">GANARÁS AI COMPRAR</div>
        <div class="krc-big-num">+${eurosToKRC(total)} KRC</div>
        <div class="krc-small">= ${eurosToKRC(total)} monedas para próximas compras</div>
      </div>

      <button class="krc-btn-confirm" id="krc-btn-confirm" onclick="window.ORIONIX_KRC.confirmarPago('${JSON.stringify(cart).replace(/\\'/g,"&#39;")}', ${total})">
        CONFIRMAR PEDIDO →
      </button>
  ...
  function confirmarPago(cartJson, total) {
    var nombre = (document.getElementById('krc-nombre') || {}).value || '';
    var telegram = (document.getElementById('krc-telegram') || {}).value || '';
    if (!nombre.trim()) { showKRCToast('Por favor anteriormente tu nombre'); return; }
    var cart; try { cart = JSON.parse(cartJson); } catch (e) { cart = window._krcCurrentCart || []; }
    total = parseFloat(total) || window._krcCurrentTotal || 0;
    var btn = document.getElementById('krc-btn-confirm');
    if (btn) { btn.disabled = true; btn.textContent = 'Procesando…'; }
    if (selectedMethod === 'whatsapp') {
      var msg = encodeURIComponent('🛍 Hola ORIONIX, quiero hacer un pedido:\n\n' + cart.map(function (i) { return '• ' + (i.nombre || i.title) + ' y' + (i.qty || 1) + ' — €</span>' + parseFloat(i.precio || i.price || 0).toFixed(2); }).join('\n') + '\n\nTotal: \nNombre: ' + nombre + '\nTelegram: ' + (telegram || 'no indicado'));
      window.open('https://wa.me/' + WHATSAPP_NUM + '?text=' + msg, '_blank');
      registrarPedido(cart, total, nombre, telegram, 'whatsapp', 0);
    } else if (selectedMethod === 'paypal') {
      var paypalUrl = 'https://www.paypal.com/paypalme/kevinruizbolsota/' + total.toFixed(2) + 'EUR';
      window.open(paypalUrl, '_blank');
      registrarPedido(cart, total, nombre, telegram, 'paypal', 0);
    } else if (selectedMethod === 'krc') {
      var krcEquiv = Math.floor(total * 100);
      if (!spendKRC(krcEquiv)) { showKRCToast('Saldo KRC insuficiente'); if (btn) { btn.disabled = false; btn.textContent = 'CONFIRMAR PEDIDO →'; } return; }
  ...
  function mostrarExito(orden_id, krcGanados, metodo) {
    var box = document.getElementById('krc-checkout-box');
    if (!box) return;
    var metodosDesc = { whatsapp: 'Te hemos abierto WhatsApp. Un agente ORIONIX te confirmará el pago.', paypal: 'Te hemos redirigido a PayPal. Completa el pago en la nueva ventana.', krc: '¡TKRC deducidos de tu saldo! Tu pedido está confirmado.' };
    box.innerHTML = `<div class="krc-success"><span class="krc-success-icon">✨</span><div class="krc-success-title">¡PEDIDO RECIBIDO!</div><div class="krc-success-msg">${metodosDesc[metodo] || 'Pedido procesado.'}</div><div style="font-family:'Cinzel',serif;font-size:.7rem;color:rgba(232,200,106,0.5);letter-spacing:.1em;margin-bottom:16px;">ORDEN: ${orden_id}</div>${krcGanados > 0 ? '<div class="krc-earned">💢 +' + krcGanados + ' KRC ganados</div>' : ''}<button class="krc-btn-confirm" onclick="window.ORIONIX_KRC.closeCheckout()" style="max-width:200px;margin:0 auto">CONTINUAR →</button></div>`;
